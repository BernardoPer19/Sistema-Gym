...
interface CheckInScannerProps {
  onCheckIn: (code: string) => { success: boolean; member?: Member; message: string }
}

export function CheckInScanner({ onCheckIn }: CheckInScannerProps) {
  const [code, setCode] = useState("")
  const [result, setResult] = useState<{ success: boolean; member?: Member; message: string } | null>(null)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const checkInResult = onCheckIn(code)
  ...
  const handleStartScanning = () => {
    ...
    toast({
      ...
      description: "En producción, esto activaría la cámara para escanear códigos QR",
    })
    ...
    <Card className="bg-card border-border">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          {scanMode === "qr" ? "Escanea el código QR del socio" : "Ingresa el código del socio"}
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
          ...
          <div className="flex flex-col items-center gap-4 rounded-lg border-2 border-dashed border-neutral-800 bg-neutral-900 p-8">
            {isScanning ? (
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-sm text-neutral-400">Escaneando código QR...</p>
              </div>
            ) : (
              <>
                ...
                <p className="text-xs text-neutral-500">O pega el código QR abajo</p>
              </>
            )}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="flex gap-2">
          <Input
            placeholder={
              scanMode === "qr" ? "Escanea o pega el código QR..." : "Ingresa código (ej: JUAN-19900515-001)"
            }
            value={code}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className={`font-semibold ${result.success ? "text-green-500" : "text-red-500"}`}>
                  {result.success ? "✓ Asistencia Registrada" : "✗ Acceso Denegado"}
                </p>
                {result.member && (
                  <div className="mt-2 space-y-1">
                    <div className="flex items-center gap-2">
                      <User className="h-4 w-4 text-muted-foreground" />
                      <span className="text-white font-medium">{result.member.name}</span>
                    </div>
                    <p className="text-sm text-muted-foreground">{result.message}</p>
                  </div>
                )}
                {!result.member && <p className="text-sm text-muted-foreground mt-1">{result.message}</p>}
              </div>
            </div>
          </div>
        )}
